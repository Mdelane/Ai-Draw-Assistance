...
export default function OutfitterProfilePage() {
  ...
  if (loading) return (
    <>
      <Navbar />
      <div className="max-w-lg mx-auto px-6 py-16 text-center text-stone-400">Loading...</div>
    </>
  )
  ...
  return (
    <>
      ...
      <main className="max-w-lg mx-auto px-6 py-10">
        ...
        <p className="text-stone-500 text-sm mb-8">This is what hunters see on your listings.</p>
...
const input = 'w-full border border-stone-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-amber-400'

function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div>
      <label className="block text-xs font-bold uppercase tracking-widest text-stone-600 mb-1.5">{label}</label>
      {children}
    </div>
  )
}
