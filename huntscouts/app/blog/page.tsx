  ...
  return (
    <>
      ...
      <main className="max-w-5xl mx-auto px-6 py-16">
        <div className="mb-12">
          <p className="text-xs font-bold uppercase tracking-widest text-[#1B4332] mb-3">Field Notes</p>
          <h1 className="text-5xl font-black tracking-tight text-gray-900 mb-3">Western Hunting Strategy Blog</h1>
          <p className="text-gray-500 text-lg">Draw odds breakdowns, unit comparisons, and outfitter advice from HuntScouts.</p>
        </div>

        {posts.length === 0 ? (
          <div className="bg-stone-50 border border-dashed border-stone-200 rounded-xl p-16 text-center text-gray-400">
                ...
                <div className="flex items-start justify-between gap-6">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      <span className={`text-xs font-bold px-2.5 py-0.5 rounded-full ${CATEGORY_COLORS[post.category] ?? 'bg-gray-100 text-gray-700'}`}>
                        {post.category}
                      </span>
                      <span className="text-xs text-gray-400">{post.readingTime}</span>
                    </div>
                    <h2 className="text-xl font-black text-gray-900 mb-2 group-hover:text-[#1B4332] transition-colors">{post.title}</h2>
